...
static void sgm7220_connect_state_notify(struct sgm7220_info *info)
{
	...
	pr_debug("%s attach_state=%d, pre_state=%d\n", __func__, attach_state, pre_state);
...
static int sgm7220_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = request_threaded_irq(client->irq, NULL, sgm7220_irq_thread,
				   IRQF_TRIGGER_LOW | IRQF_ONESHOT, "sgm7220_irq", info);
	if (ret) {
	...
	dev_info(&client->dev, "sgm7220 usb type-c ship finish probe\n");
	return 0;
