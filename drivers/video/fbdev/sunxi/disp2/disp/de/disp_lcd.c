...
#include "disp_lcd.h"
//add by yxp yanjb 20210401
#include "disp_display.h"
#include "../dev_disp.h"

extern u32 suspend_status; 
//end add by yxp yanjb 20210401   
extern void lcd_set_panel_funs(void);
#define DISP_LCD_MODE_DIRTY_MASK 0x80000000


#define DBG_INFO(format, args...) (printk("[DISP LCD INFO] LINE:%04d-->%s:"format, __LINE__, __func__, ##args))
#define DBG_ERR(format, args...) (printk("[DISP LCD ERR] LINE:%04d-->%s:"format, __LINE__, __func__, ##args))
...
static s32 lcd_clk_config(struct disp_device *lcd)
{
	...
	if (lcdp->panel_info.lcd_if == LCD_IF_DSI && lcdp->panel_info.lcd_dsi_lane == 2) {
...
static s32 disp_lcd_backlight_enable(struct disp_device *lcd)
{
	...
	if (disp_lcd_is_used(lcd)) {
		...
		disp_lcd_set_bright(lcd, bl);     //yanjb 20210402
	}
...
int backlight_bright_temp_yxp=0;
EXPORT_SYMBOL(backlight_bright_temp_yxp);

static int bright_temp_yxp = 0;
struct disp_device *yxp_lcd; //yanjb 20210403
int disp_lcd_get_brightness_yxp(int bright_temp_yxp)
{
	if (yxp_lcd == NULL) {
		DE_WRN("param is NULL!\n");
		return NULL;
	}
//	printk("==yy==disp_lcd_get_brightness_yxp = %d \n",bright_temp_yxp);
	msleep(1);
	bright_temp_yxp = 0xff - bright_temp_yxp;
    disp_lcd_set_bright(yxp_lcd,bright_temp_yxp);
    return 0;
}
EXPORT_SYMBOL( disp_lcd_get_brightness_yxp );	
#endif

#if defined (CONFIG_YOUXUEPAI_BOARD) || defined (CONFIG_COWBOY_BOARD)
extern int low_bl_flag;
#endif
...
#if defined (CONFIG_YOUXUEPAI_BOARD) || defined (CONFIG_COWBOY_BOARD)
	__u64 backlight_bright_temp = bright;
#endif
...
#if defined (CONFIG_YOUXUEPAI_BOARD) || defined (CONFIG_COWBOY_BOARD)
    if(low_bl_flag){   //上层通过独立接口设置长时间不操作；屏幕变暗。此光感值不保存；
		backlight_bright = backlight_bright_temp;
		bright_update = true;
		low_bl_flag = 0;
    	}else{
#endif	
...
#if defined (CONFIG_YOUXUEPAI_BOARD) || defined (CONFIG_COWBOY_BOARD)
		    backlight_bright_temp_yxp = 0xff - backlight_bright;
#endif
       		}
#if defined (CONFIG_YOUXUEPAI_BOARD) || defined (CONFIG_COWBOY_BOARD)
    		}	
...
#if defined (CONFIG_YOUXUEPAI_BOARD) || defined (CONFIG_COWBOY_BOARD)
//ioctr brightness 1-100  add by yxp 20210226
		backlight_bright = 0xff - backlight_bright;
		backlight_bright = backlight_bright*71/255;  // yanjb 背光亮度整体降低28%	 
	
		backlight_bright = backlight_bright*65/71;
		backlight_bright += 6;
		backlight_bright = 0xff - backlight_bright;
#endif
