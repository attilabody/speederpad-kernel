...
void set_ts_set_brightness_yxp(unsigned long tmpLevel);    //yanjb20210111
int get_ts_get_brightness_yxp(void);
#endif
...
#if defined (CONFIG_YOUXUEPAI_BOARD) || defined (CONFIG_COWBOY_BOARD)
//yanjb202101011
int al3010_bl_flag_yxp=0;
EXPORT_SYMBOL(al3010_bl_flag_yxp);
int al3010_bl_level=0;
EXPORT_SYMBOL(al3010_bl_level);

int yxp_set_low_brightness=0;
int low_bl_flag=0;
static ssize_t disp_brightness_sys_show(struct device *dev,
			     struct device_attribute *attr, char *buf)
{
     unsigned long tmpLevel; 

     tmpLevel = get_ts_get_brightness_yxp();
	 tmpLevel = 255 - tmpLevel;	
	 if (tmpLevel <= 0)
	 	 tmpLevel = 0;
	 else
	     tmpLevel = tmpLevel*100/255;	 
	 
	 if ((tmpLevel == 0)||(tmpLevel==100))
	 	 tmpLevel = tmpLevel;
	 else
	 	 tmpLevel += 1;
...
static ssize_t disp_brightness_sys_store(struct device *dev,
			      struct device_attribute *attr,
			      const char *buf, size_t count)
{  
	unsigned long tmpLevel;

    sscanf(buf, "%lu", &tmpLevel);
	if(tmpLevel > 100)
	   tmpLevel = (tmpLevel > 100) ? 100 : tmpLevel;
	if (tmpLevel <= 0){
		tmpLevel = 0;
	    tmpLevel = 255 - tmpLevel;
	}else{
	    tmpLevel = tmpLevel * 255/100;    //100---255
	    tmpLevel = 255 - tmpLevel;
		}
	set_ts_set_brightness_yxp(tmpLevel);

	return count;
}

static ssize_t disp_set_low_brightness_sys_show(struct device *dev,
			     struct device_attribute *attr, char *buf)
{
	 return sprintf(buf, "%u\n", yxp_set_low_brightness);

}
static ssize_t disp_set_low_brightness_sys_store(struct device *dev,
			      struct device_attribute *attr,
			      const char *buf, size_t count)
{  
	int var;
	...
	printk("var=%d\n", var);
    low_bl_flag=1;
	yxp_set_low_brightness = var;
	yxp_set_low_brightness = 0xff - yxp_set_low_brightness;
	set_ts_set_brightness_yxp(yxp_set_low_brightness);
	
	return count;
}

static ssize_t disp_brightness_default_sys_show(struct device *dev,
			     struct device_attribute *attr, char *buf)
{
     unsigned long tmpLevel; 

     tmpLevel = get_ts_get_brightness_yxp();
     tmpLevel = 0xff - tmpLevel;
     return sprintf(buf, "Brightness = %d\n", tmpLevel);

}
static ssize_t disp_brightness_default_sys_store(struct device *dev,
			      struct device_attribute *attr,
			      const char *buf, size_t count)
{  
	unsigned long tmpLevel;
    sscanf(buf, "%lu", &tmpLevel);
	tmpLevel = 0xff - tmpLevel;     //yanjb 20210420
...
static DEVICE_ATTR(disp_brightness, 0660,
		   disp_brightness_sys_show, disp_brightness_sys_store);
static DEVICE_ATTR(disp_set_low_brightness, 0660,
		   disp_set_low_brightness_sys_show, disp_set_low_brightness_sys_store);
static DEVICE_ATTR(disp_brightness_default, 0660,
		   disp_brightness_default_sys_show, disp_brightness_default_sys_store);
...
static struct attribute *disp_attributes[] = {
	&dev_attr_sys.attr,
#if defined (CONFIG_YOUXUEPAI_BOARD) || defined (CONFIG_COWBOY_BOARD)
	&dev_attr_disp_brightness.attr,
	&dev_attr_disp_set_low_brightness.attr,
	&dev_attr_disp_brightness_default.attr,
#endif 
	&dev_attr_disp.attr,
...
//yanjb20210111
int bl_state = 0;
EXPORT_SYMBOL(bl_state);
extern int backlight_bright_temp_yxp;

void set_ts_set_brightness_yxp(unsigned long tmpLevel)
{
    s32 ret = 0;
	struct disp_device *dispdev =  g_disp_drv.mgr[0]->device;

	if (dispdev && dispdev->set_bright)
				ret = dispdev->set_bright(dispdev, tmpLevel);
...
int get_ts_get_brightness_yxp(void)
{
	...
	ret = dispdev->get_bright(dispdev);
              pr_info("DISP_LCD_GET_BRIGHTNESS get_bright() = %d \n", ret);
	...
	case DISP_LCD_SET_BRIGHTNESS:
		{
			if (dispdev && dispdev->set_bright)
            {
                int bright = 100;
#if defined (CONFIG_YOUXUEPAI_BOARD) || defined (CONFIG_COWBOY_BOARD)
                al3010_bl_flag_yxp = 0;   //yanjb20210408
	            al3010_bl_level = 0;
                if(bl_state){  //打开自动调节开关，当ioctl传下buffer参数为0时。设置为灭屏。
					if(ubuffer[1] == 0)
						ubuffer[1] = 0;
					else
				        ubuffer[1] = backlight_bright_temp_yxp;
                	}
				pr_info("ioctl set bl ubuffer[1] = %d\n", ubuffer[1]);
#endif				
				if( (ubuffer[1] >= 0) && (ubuffer[1] <= 255) )
                    bright = 0xff - ubuffer[1];
				ret = dispdev->set_bright(dispdev, bright);
            }
			break;
		}
