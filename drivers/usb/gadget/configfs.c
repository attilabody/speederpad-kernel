...
static int yxp_usbstate = 0;
static ssize_t gadget_dev_desc_USBstate_show(struct config_item *item, char *page)
{
	if(yxp_usbstate ==1)
		return sprintf(page, "%s\n", "CONNECTED");
	else
...
static ssize_t gadget_dev_desc_USBstate_store(struct config_item *item,
		const char *page, size_t len)
{
	return 0;
}
...
CONFIGFS_ATTR(gadget_dev_desc_, USBstate);
