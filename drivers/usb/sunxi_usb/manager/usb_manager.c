...
typedef struct sunxi_usb_ext_id {
	struct extcon_dev *ext;
	struct notifier_block nb;
	usb_id_state_t id_state;
} ext_id_t;
ext_id_t g_ext_id = {
	.ext = NULL,
...
#if defined(CONFIG_EXTCON) && defined(CONFIG_EXTCON_SGM7220)
usb_id_state_t sunxi_usb_extcon_get_id_state(void)
{
	return g_ext_id.id_state;
}
EXPORT_SYMBOL(sunxi_usb_extcon_get_id_state);
extern int gpio_yxp;
static int sunxi_usb_otg_id_notifier(struct notifier_block *nb,
				unsigned long event, void *ptr)
{
	ext_id_t *ext_id = container_of(nb, struct sunxi_usb_ext_id, nb);

	pr_info("[%s] line:%d event=%d\n", __func__, __LINE__, event);
	if (event){
		ext_id->id_state = USB_HOST_MODE;
		gpio_set_value(gpio_yxp,1);
	}
	else{
		ext_id->id_state = USB_DEVICE_MODE;
		gpio_set_value(gpio_yxp,0);
	}
	return NOTIFY_DONE;
}

static int usb_extcon_parse(struct device_node *np, struct usb_cfg *cfg)
{
	if (!of_property_read_bool(np, "extcon"))
		return -1;
	g_ext_id.ext = extcon_get_edev_by_phandle(&cfg->pdev->dev, 0);
	if (IS_ERR(g_ext_id.ext)) {
		DMSG_PANIC("can't find extcon\n");
		return -ENODEV;
	}
	g_ext_id.nb.notifier_call = sunxi_usb_otg_id_notifier;
	if (extcon_register_notifier(g_ext_id.ext, EXTCON_USB_HOST, &g_ext_id.nb))
		return -1;
	/*
		static const unsigned int sgm7220_cable[] = {
			EXTCON_USB,
			EXTCON_USB_HOST, <--- index=1
			EXTCON_JACK_HEADPHONE,
			EXTCON_NONE,
		};
	*/
	if (g_ext_id.ext->state & (1 << 1))
		g_ext_id.id_state = USB_HOST_MODE;
...
static __s32 usb_script_parse(struct device_node *np, struct usb_cfg *cfg)
{
	...
	if (ret) {
		...
		if (strncmp(cfg->port.id_name, "axp_ctrl", 8) == 0) {
			...
			cfg->port.id.valid = 0;
#if defined(CONFIG_EXTCON) && defined(CONFIG_EXTCON_SGM7220)
		} else if (strncmp(cfg->port.id_name, "extcon", 6) == 0) {
			cfg->port.id_type = USB_ID_TYPE_EXTCON;
			usb_extcon_parse(np, cfg);
#endif
		} else {
			/*get id gpio */
