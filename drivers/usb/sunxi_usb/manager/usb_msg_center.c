...
static void rmmod_device_driver(struct usb_msg_center_info *center_info)
{
	...
	//printk("FILE:%s, FUNC:%s, LINE:%d, psy:%p\n", __FILE__, __func__, __LINE__, psy);
	if (!psy || IS_ERR(psy)) {
		DMSG_PANIC("%s()%d WARN: get power supply failed\n",
			  __func__, __LINE__);
	} else {
		if (of_property_read_u32(psy->of_node, "pmu_usbpc_cur", &temp.intval)){		
			temp.intval = 0;
		}
		//printk("FILE:%s, FUNC:%s, LINE:%d, intval:%d\n", __FILE__, __func__, __LINE__, temp.intval);		
