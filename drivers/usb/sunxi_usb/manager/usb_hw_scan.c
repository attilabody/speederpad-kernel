...
static u32 get_id_state(struct usb_scan_info *info)
{
	...
	} else if (info->cfg->port.id_type == USB_ID_TYPE_EXTCON) {
extern usb_id_state_t sunxi_usb_extcon_get_id_state(void);
		id_state = sunxi_usb_extcon_get_id_state();
#endif
	} else {
		id_state = info->id_old_state;
	}
...
__s32 usb_hw_scan_init(struct usb_cfg *cfg)
{
...
#else
	{
		...
		if (port_info->det_vbus_type == USB_DET_VBUS_TYPE_AXP) {
			if (of_find_property(cfg->pdev->dev.of_node,
						"det_vbus_supply", NULL)) {
				port_info->psy = devm_power_supply_get_by_phandle(&cfg->pdev->dev,
						"det_vbus_supply");
				if (!(port_info->psy) || IS_ERR(port_info->psy)) {
					DMSG_INFO("%s()%d WARN: "
