...
#if defined(CONFIG_ARCH_SUN50IW11)
	if (sunxi_hci->usbc_no == HCI1_USBC_NO) {
		usb_new_phyx_tp_write(sunxi_hci, 0x16, 0x03, 0x03);
		pr_debug("addr:%x,len:%x,value:%x\n", 0x16, 0x03,
		       usb_new_phyx_tp_read(sunxi_hci, 0x16, 0x03));
		usb_new_phyx_tp_write(sunxi_hci, 0x0b, 0x50, 0x08);
		pr_debug("addr:%x,len:%x,value:%x\n", 0x0b, 0x08,
		       usb_new_phyx_tp_read(sunxi_hci, 0x0b, 0x08));
		usb_new_phyx_tp_write(sunxi_hci, 0x09, 0x02, 0x02);
		pr_debug("addr:%x,len:%x,value:%x\n", 0x09, 0x02,
		       usb_new_phyx_tp_read(sunxi_hci, 0x09, 0x02));
		usb_new_phyx_tp_write(sunxi_hci, 0x03, 0x03, 0x06);
		pr_debug("addr:%x,len:%x,value:%x\n", 0x03, 0x06,
		       usb_new_phyx_tp_read(sunxi_hci, 0x03, 0x06));
		usb_new_phyx_tp_write(sunxi_hci, 0x1c, 0x07, 0x03);
		pr_debug("addr:%x,len:%x,value:%x\n", 0x1c, 0x03,
		       usb_new_phyx_tp_read(sunxi_hci, 0x1c, 0x03));
		usb_new_phyx_tp_write(sunxi_hci, 0x30, 0x33, 0x0d);
		pr_debug("addr:%x,len:%x,value:%x\n", 0x30, 0x0d,
		       usb_new_phyx_tp_read(sunxi_hci, 0x30, 0x0d));
		usb_new_phyx_tp_write(sunxi_hci, 0x60, 0x23b8, 0x0e);
		pr_debug("addr:%x,len:%x,value:%x\n", 0x60, 0x0e,
		       usb_new_phyx_tp_read(sunxi_hci, 0x60, 0x0e));
		usb_new_phyx_tp_write(sunxi_hci, 0x92, 0x04, 0x03);
		pr_debug("addr:%x,len:%x,value:%x\n", 0x92, 0x03,
		       usb_new_phyx_tp_read(sunxi_hci, 0x92, 0x03));
	}
#endif
