...
#define WSM_MIB_ID_HOST_SLEEP_FILTER    0x1047
...
struct wsm_sleep_filter_s {
	u16 filter_enable;
	u16 enther_type;
	u8  ip_proto;
	u8  ip_proto_type;
	u16 ip_proto_type_off;
	u8  mac_addr_s[6];
	u8  ip_addr_s[4];
};

static inline int wsm_set_host_sleep_fliter(struct xradio_common *hw_priv,
		struct wsm_sleep_filter_s *host_sleep_filter,
		int if_id)
{
	return wsm_write_mib(hw_priv, WSM_MIB_ID_HOST_SLEEP_FILTER,
			(void *)host_sleep_filter,
			sizeof(struct wsm_sleep_filter_s), if_id);
}

#if HOST_SLEEP_PING_FILTER
static inline int wsm_set_host_sleep_ping_filter(struct xradio_common *hw_priv,
			struct wsm_sleep_filter_s *ping_filter,
			u16 enable, u8 *mac_addr, u8 *ip_addr, int if_id)
{
	ping_filter->filter_enable     = enable;
	ping_filter->enther_type       = 0xffff;   //IP
	ping_filter->ip_proto          = 0xff;
	memset(ping_filter->mac_addr_s, 0, 6);
	memset(ping_filter->mac_addr_s, 0, 4);

	return wsm_set_host_sleep_fliter(hw_priv, ping_filter, if_id);
}
#endif /* HOST_SLEEP_PING_FILTER */
