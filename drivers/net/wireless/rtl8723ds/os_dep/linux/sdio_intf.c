...
static struct device *wdev;
extern struct device *sunxi_wlan_get_dev(void);
...
static irqreturn_t gpio_hostwakeup_irq_thread(int irq, void *data)
{
	...
	return 0;
#else
	if (wdev && device_may_wakeup(wdev))
		pm_wakeup_event(wdev, 0);
...
static u8 gpio_hostwakeup_alloc_irq(PADAPTER padapter)
{
	...
	RTW_INFO("call  enable_irq_wake.irq number: %d ok\n", oob_irq);
...
#if !defined(CONFIG_PLATFORM_ARM_SUN8I) && !defined(CONFIG_ARCH_SUN50IW11P1) && !defined(CONFIG_ARCH_SUN50IW10P1)
	RTW_INFO("call  disable_irq_wake.irq number: %d ok\n", oob_irq);
...
static int __init rtw_drv_entry(void)
{
	...
	/*get wakeup device for wlan*/
	wdev = sunxi_wlan_get_dev();
