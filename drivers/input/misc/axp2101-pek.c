...
/*for sleep  test input keyevent */
struct input_dev *input_power_dev;//add by kfliu

static ssize_t input_power_event_store(struct device *cd,
    struct device_attribute *attr,
    const char *buf, size_t len)
{
	...
	if(input_power_dev){
		input_report_key(input_power_dev, KEY_POWER, true);
		input_sync(input_power_dev);
		msleep(50);
		input_report_key(input_power_dev, KEY_POWER, false);
		input_sync(input_power_dev);
	}
...
static DEVICE_ATTR(input_power_event, 0660, NULL, input_power_event_store);
static struct kobject *input_power_event_kobj=NULL;
static int create_input_power_event(void)
{
	int liRet;
	input_power_event_kobj = kobject_create_and_add("input_power", NULL);
    if (input_power_event_kobj == NULL){
            printk(KERN_ERR"input_power_event_kobj create error!");
    }

    liRet = sysfs_create_file(input_power_event_kobj, &dev_attr_input_power_event.attr); 
    if(liRet < 0){
            printk(KERN_ERR"input_power_event create error!");
            kobject_del(input_power_event_kobj);
            return -1;
    }
	return 0;
}
...
static int axp803_config_set(struct axp20x_pek *axp20x_pek)
{
	...
	/*Init offlevel restart or not */
	if (pk_dts->pmu_powkey_off_func) {
		regmap_update_bits(regmap, AXP803_POK_SET, (0x01 << 2),
				   (0x01 << 2)); /* restart */
	}
	else {
		regmap_update_bits(regmap, AXP803_POK_SET, (0x01 << 2),
...
static int axp20x_pek_probe(struct platform_device *pdev)
{
	...
	input_power_dev = idev;
	create_input_power_event();
