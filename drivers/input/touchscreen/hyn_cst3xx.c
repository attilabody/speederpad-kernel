...
static int hard_reset_chip(int ms);     //yanjb 20210425
...
struct hyn_fw_array {
	...
	const unsigned char *fw;
     ...
     { "hyn_cst3xx_fx_Ironman_7cun_fw",  "CPH_2008023_CST340_HYN_FXXC070G_0594_0701", 0x1737  ,cst3_fw },         //0x1823
...
static int test_i2c(struct i2c_client *client)
{
	...
	while(retry++<5) {
		...
		if (ret > 0)
			...
			hard_reset_chip(30);   //yanjb20210212 reset
		msleep(2);		
	}
   ...
   return -1;
	...
	if((strict > 0)&&(g_cst3xx_ic_project_id == project_id)){
		...
		if (g_cst3xx_ic_checksum != fw_checksum){
			...
			if (g_cst3xx_ic_version != fw_version){      //判断固件升级条件    yanjb			
				return 0;
			}else{
				return -1;
			}
...
static int hyn_boot_update_fw(struct i2c_client * client)
{
	...
	if(ret==0){
	    ...
	    ptr_fw = cst3xx_fw_grp[0].fw; 
		HYN_INF("ic have no firmware, please return to module supplier!\n");
		cst3xx_update_firmware(client, ptr_fw);    //yanjb 20210427 防止模组厂漏烧固件，强制升级固件
//		return -1;	
	}
...
static ssize_t hyn_enable_store (struct device* dev, struct device_attribute* attr, const char* buf, size_t count)
{
	...
	if(ts->is_enable){
		...
		input_set_power_enable(&(config_info.input_type),1);	//yanjb 20210425
...
#endif
		ts->is_enable = 0;
	    input_set_power_enable(&(config_info.input_type),0);	//yanjb 20210425
	    __gpio_set_value(config_info.wakeup_gpio.gpio, 0);
...
static ssize_t input_event_store (struct device* dev, struct device_attribute* attr, const char* buf, size_t count)
{
	struct hyn_ts *ts = i2c_get_clientdata(cst3xx_i2c);
	int x=0,y=0,test_mode;
	
	sscanf(buf, "%d %d %d", &test_mode,&x,&y);

	printk("set touchscreen location x= %d,y=%d\n",x,y);

	if(test_mode){
		cst3xx_touch_down(ts->input,0,x,y,0x1b<<1);//x:123 y:361
		input_sync(ts->input);

		cst3xx_touch_down(ts->input,0,x,y,0x26<<1);
		input_sync(ts->input);
		cst3xx_touch_up(ts->input,0);
		input_sync(ts->input);
	}else{
		x=80;
		cst3xx_touch_down(ts->input,0,x,788,0x1b<<1);
		input_sync(ts->input);
		cst3xx_touch_down(ts->input,0,x,755,0x1b<<1);
		input_sync(ts->input);
		cst3xx_touch_down(ts->input,0,x,730,0x1b<<1);
		input_sync(ts->input);
		cst3xx_touch_down(ts->input,0,x,704,0x1b<<1);
		input_sync(ts->input);
		cst3xx_touch_down(ts->input,0,x,644,0x1b<<1);
		input_sync(ts->input);
		cst3xx_touch_up(ts->input,0);
		input_sync(ts->input);
	}
...
static DEVICE_ATTR (input_event, S_IRUGO | S_IWUSR, NULL,
										input_event_store);
