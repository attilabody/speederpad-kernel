...
static void set_sensor_isp_used(struct sensor_list *sensors,
		void *value, int sel)
{
	sensors->inst[sel].is_isp_used = *(int *)value;
}
static void set_sensor_is_bayer_raw(struct sensor_list *sensors,
		void *value, int sel)
{
	sensors->inst[sel].is_bayer_raw = *(int *)value;
}
...
static struct SensorParamAttribute SensorParamDetect[] = {
	...
	{"sensor_isp_used", 1, INTEGER, set_sensor_isp_used,},
	{"sensor_is_bayer_raw", 1, INTEGER, set_sensor_is_bayer_raw,},
...
static int parse_sensor_list_info(struct sensor_list *sl, char *pos)
{
	...
	sprintf(sensor_list_cfg, "/etc/config/sensor_list_cfg.ini");
