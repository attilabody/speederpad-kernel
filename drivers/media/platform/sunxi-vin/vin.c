...
static int vin_pipeline_s_power(struct vin_pipeline *p, bool on)
{
	static const u8 seq[2][VIN_IND_MAX] = {
		...
		{ VIN_IND_CAPTURE, VIN_IND_MIPI, VIN_IND_CSI,
			VIN_IND_ISP, VIN_IND_SCALER, VIN_IND_SENSOR},
	};
	int i, ret = 0;
...
static int __vin_pipeline_s_stream(struct vin_pipeline *p, int on_idx)
{
	static const u8 seq[3][VIN_IND_MAX] = {
		...
		{ VIN_IND_TDM_RX, VIN_IND_MIPI, VIN_IND_ISP,
			VIN_IND_SCALER, VIN_IND_CAPTURE, VIN_IND_CSI, VIN_IND_SENSOR},
		{ VIN_IND_TDM_RX, VIN_IND_MIPI, VIN_IND_ISP,
			VIN_IND_SCALER, VIN_IND_CAPTURE, VIN_IND_CSI, VIN_IND_SENSOR},
	};
			...
			char *name, u8 addr, enum module_type type, int bus_sel)
{
	...
	if (type == VIN_MODULE_TYPE_CCI) {
		...
		if (IS_ERR_OR_NULL(sd)) {
			vin_err("cci registering %s, No such device!\n", name);
			return NULL;
		} else {
		...
		if (IS_ERR_OR_NULL(sd)) {
			...
			vin_err("i2c registering %s, No such device!\n", name);
			return NULL;
		} else {
			vin_log(VIN_LOG_MD, "%s register OK!\n", name);
		}
		...
		if (IS_ERR_OR_NULL(sd)) {
			...
			vin_err("spi registering %s, No such device!\n", name);
			return NULL;
		} else {
			vin_log(VIN_LOG_MD, "%s register OK!\n", name);
		}
