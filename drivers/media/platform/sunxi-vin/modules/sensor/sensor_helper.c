...
int flash_en(struct v4l2_subdev *sd, struct flash_para *para)
{
	...
	printk("FILE:%s, FUNC:%s, LINE:%d, modules:%p, flash_sd:%p, mode:0x%x\n", __FILE__, __func__, __LINE__, modules, modules ? modules->modules.flash.sd : NULL, para->mode);
	if (modules == NULL)
		return -EINVAL;
...
static int sensor_helper_probe(struct platform_device *pdev)
{
	...
	snprintf(power_supply, sizeof(power_supply), "sensor%d_afvdd", pdev->id);
	sensor_helper->pmic[AFVDD] = regulator_get_optional(dev, power_supply);
	if (IS_ERR(sensor_helper->pmic[AFVDD])) {
		vin_warn("%s: cannot get %s supply, setting it to NULL!\n", __func__, power_supply);
		sensor_helper->pmic[AFVDD] = NULL;
	}
