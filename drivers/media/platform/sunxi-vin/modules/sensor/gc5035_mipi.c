/*
 * A V4L2 driver for gc5035_mipi cameras.
 *
...
/*
 * The gc5035_mipi sits on i2c with ID 0x6e
 */
...
/* 5M Raw10 30fps 24MHz */
static struct regval_list sensor_QSXGA_30fps_regs[] = {
	/* System */
	{0xfe, 0x00},
	{0x3e, 0x01},
	{0xfc, 0x01},
...
static int sensor_s_exp(struct v4l2_subdev *sd, unsigned int exp_val)
{
	...
	if (exp_val > ((gc5035_sensor_vts - 16) << 4))
		exp_val = (gc5035_sensor_vts - 16) << 4;
	if (exp_val < 16)
		exp_val = 16;
...
{
	unsigned short gain_tmp;
	unsigned char gain_b6, gain_b1, gain_b2;
	struct sensor_info *info = to_state(sd);
	...
	if ((gain_tmp >= ANALOG_GAIN_1) && (gain_tmp <= ANALOG_GAIN_2)) {
		gain_b6 = 0x00;
		gain_b1 = gain_tmp >> 6;
		gain_b2 = (gain_tmp << 2) & 0xfc;
	} else {
		gain_b6 = 0x01;
		gain_tmp = gain_tmp * 64 / ANALOG_GAIN_2;
		gain_b1 = gain_tmp >> 6;
		gain_b2 = (gain_tmp << 2) & 0xfc;
	}

	sensor_write(sd, 0xfe, 0x00);
	sensor_write(sd, 0xb6, gain_b6);
	sensor_write(sd, 0xb1, gain_b1);
	sensor_write(sd, 0xb2, gain_b2);

	sensor_dbg("sensor_s_gain = %d\n", gain_val);
	info->gain = gain_val & 0x03FF;
...
static int sensor_detect(struct v4l2_subdev *sd)
{
	...
	if (rdval != (V4L2_IDENT_SENSOR >> 8)) {
		sensor_err(" read 0xf0 return 0x%x\n", rdval);
		return -ENODEV;
	}

	sensor_read(sd, 0xf1, &rdval);
	if (rdval != (V4L2_IDENT_SENSOR & 0xff)) {
		sensor_err(" read 0xf1 return 0x%x\n", rdval);
		return -ENODEV;
	}
...
//		.mbus_code = MEDIA_BUS_FMT_SBGGR10_1X10,
		.mbus_code = MEDIA_BUS_FMT_SBGGR10_1X10,  //0202 sensor_output_dataformat
		.regs = sensor_fmt_raw,
...
static struct sensor_win_size sensor_win_sizes[] = {
	{
		...
		.gain_min   = 1 << 4,
		.gain_max   = 256 << 4,
		.regs       = sensor_QSXGA_30fps_regs,
		.regs_size  = ARRAY_SIZE(sensor_QSXGA_30fps_regs),
		.set_size   = NULL,
	},
};
