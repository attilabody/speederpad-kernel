				...
				unsigned short halfword, void *pt)
{
	...
	for (count = 0; count < ACT_WRITE_MAX_NUM; count++) {
		ret = cci_write_a0_d16(&act_ctrl->sdev, halfword);
		if (ret < 0)
			act_err("subdev_i2c_write write 0x%x err!\n ", halfword);
		act_dbg("set value = [0x%x]\n", halfword);
		ret = cci_read_a0_d16(&act_ctrl->sdev, &read_value);
		if (ret < 0)
			act_err("subdev_i2c_write read err!\n ");
		if (read_value == halfword)
			break;
	}
/*
	if (count >= ACT_WRITE_MAX_NUM)
		act_err("subdev_i2c_write write 0x%x err, count %d\n ", halfword, count);
*/
	return ret;
}
...
#ifdef DLC
	ret = subdev_i2c_write(act_ctrl, new_code << 4, NULL);
	goto set_code_ret;
#else
			...
			unsigned short mode)
{
	...
	if (mode == 1) {
		...
		act_ctrl->work_status = ACT_STA_HALT;
		if (act_ctrl->curr_code > 700) {
			subdev_i2c_write(act_ctrl, 680 << 4, NULL);
			usleep_range(13000, 15000);
			subdev_i2c_write(act_ctrl, 550 << 4, NULL);
			usleep_range(13000, 15000);
			subdev_i2c_write(act_ctrl, 400 << 4, NULL);
			usleep_range(13000, 15000);
		} else if (act_ctrl->curr_code > 550 && act_ctrl->curr_code <= 700) {
			subdev_i2c_write(act_ctrl, 550 << 4, NULL);
			usleep_range(13000, 15000);
			subdev_i2c_write(act_ctrl, 400 << 4, NULL);
			usleep_range(13000, 15000);
		} else if (act_ctrl->curr_code > 350 && act_ctrl->curr_code <= 550) {
			subdev_i2c_write(act_ctrl, 380 << 4, NULL);
			usleep_range(13000, 15000);
		}
		subdev_i2c_write(act_ctrl, 300 << 4, NULL);
		usleep_range(13000, 15000);
		subdev_i2c_write(act_ctrl, 275 << 4, NULL);
		usleep_range(13000, 15000);
		subdev_i2c_write(act_ctrl, 250 << 4, NULL);
		usleep_range(13000, 15000);
		subdev_i2c_write(act_ctrl, 225 << 4, NULL);
		usleep_range(13000, 15000);
		subdev_i2c_write(act_ctrl, 200 << 4, NULL);
		usleep_range(13000, 15000);
		subdev_i2c_write(act_ctrl, 175 << 4, NULL);
		usleep_range(13000, 15000);
		subdev_i2c_write(act_ctrl, 150 << 4, NULL);
		usleep_range(13000, 15000);
		subdev_i2c_write(act_ctrl, 125 << 4, NULL);
		//ret = subdev_set_code(act_ctrl, act_ctrl->active_min, 0);
		usleep_range(13000, 15000);
		subdev_i2c_write(act_ctrl, 90 << 4, NULL);
		usleep_range(13000, 15000);
		//ret = subdev_i2c_write(act_ctrl, 0x8000, 0);
		act_ctrl->curr_code = 100;
