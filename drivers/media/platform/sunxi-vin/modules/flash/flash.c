...
static volatile int flash_power_flag;

int io_set_flash_ctrl(struct v4l2_subdev *sd, enum sunxi_flash_ctrl ctrl)
{
	int ret = 0;
	int err = 0;
#if defined (CONFIG_YOUXUEPAI_BOARD) || defined (CONFIG_COWBOY_BOARD)
	unsigned int pwm_control_reg_temp = 0;
#endif
	...
	printk("FILE:%s, FUNC:%s, LINE:%d, flash_en:%d, flash_dis:%d, flash_mode:%d, torch_mode:%d\n", __FILE__, __func__, __LINE__, flash_en, flash_dis, flash_mode, torch_mode);
	if (fls_info->flash_driver_ic == FLASH_RELATING) {
		switch (ctrl) {
		case SW_CTRL_FLASH_OFF:
			vin_log(VIN_LOG_FLASH, "FLASH_RELATING SW_CTRL_FLASH_OFF\n");
			printk("FILE:%s, FUNC:%s, LINE:%d, SW_CTRL_FLASH_OFF:\n", __FILE__, __func__, __LINE__);
#if defined (CONFIG_YOUXUEPAI_BOARD) || defined (CONFIG_COWBOY_BOARD)
			/* set FLASH_MODE */
			vin_gpio_set_status(sd, FLASH_MODE, 1);
			err = vin_gpio_write(sd, FLASH_MODE, torch_mode);
			printk("FILE:%s, FUNC:%s, LINE:%d, SW_CTRL_FLASH_OFF: torch_mode:%d, err:%d\n", __FILE__, __func__, __LINE__, torch_mode, err);

			set_flash_led_switch(NULL, NULL, "0", strlen("0"));
#else
			pwm_disable(flash->pwm_dev);
			#if 0
		...
		case SW_CTRL_FLASH_ON:
			printk("FILE:%s, FUNC:%s, LINE:%d, SW_CTRL_FLASH_ON:\n", __FILE__, __func__, __LINE__);
			vin_log(VIN_LOG_FLASH, "FLASH_RELATING SW_CTRL_FLASH_ON\n");
#if defined (CONFIG_YOUXUEPAI_BOARD) || defined (CONFIG_COWBOY_BOARD)
			/* set FLASH_MODE */
			vin_gpio_set_status(sd, FLASH_MODE, 1);
			err = vin_gpio_write(sd, FLASH_MODE, torch_mode);
			printk("FILE:%s, FUNC:%s, LINE:%d, SW_CTRL_FLASH_ON: torch_mode:%d, err:%d\n", __FILE__, __func__, __LINE__, torch_mode, err);

			set_flash_led_switch(NULL, NULL, "1", strlen("1"));
#else
			if (flash_power_flag > 0) {
				ret = 0;
				break;
			} else if (flash_power_flag == 0) {
			...
			#endif
			flash->period_ns = 10 * 1000; /* 10us */
			flash->duty_ns = flash->period_ns * 128 / 255;
			pwm_config(flash->pwm_dev, flash->duty_ns, flash->period_ns);
			pwm_set_polarity(flash->pwm_dev, PWM_POLARITY_INVERSED);

			if (flash->pmw_control_reg) {
				pwm_control_reg_temp = readl(flash->pmw_control_reg);
				pwm_control_reg_temp = (FLASH_BRIGHTNESS << 16) | (pwm_control_reg_temp & 0xffff); /* set PWM_PUL_NUM */
				pwm_control_reg_temp |= (0x1 << 9); /* set PWM Mode */
				writel(pwm_control_reg_temp, flash->pmw_control_reg);
			}

			ret = pwm_enable(flash->pwm_dev);
			if (ret < 0){
				vin_err("pwm enable failed\n");
			} else if (flash->pmw_control_reg) {
				pwm_control_reg_temp = readl(flash->pmw_control_reg);
				pwm_control_reg_temp |= (0x1 << 10); /* set PWM pulse start */
				writel(pwm_control_reg_temp, flash->pmw_control_reg);
			}
#endif
			break;
		case SW_CTRL_TORCH_ON:
			...
			printk("FILE:%s, FUNC:%s, LINE:%d, SW_CTRL_TORCH_ON:\n", __FILE__, __func__, __LINE__);
#if defined (CONFIG_YOUXUEPAI_BOARD) || defined (CONFIG_COWBOY_BOARD)
			vin_gpio_set_status(sd, FLASH_MODE, 1);
			err = vin_gpio_write(sd, FLASH_MODE, torch_mode);
			printk("FILE:%s, FUNC:%s, LINE:%d, SW_CTRL_TORCH_ON: torch_mode:%d, err:%d\n", __FILE__, __func__, __LINE__, torch_mode, err);
			       ...
			       enum sunxi_flash_ctrl ctrl)
{
	...
	printk("FILE:%s, FUNC:%s, LINE:%d, flash:%p\n", __FILE__, __func__, __LINE__, flash);

	if (!flash)
		return 0;
	printk("FILE:%s, FUNC:%s, LINE:%d, flash_mode:%d, modules:%p\n", __FILE__, __func__, __LINE__, flash->fl_info.flash_mode, modules);
	...
	printk("FILE:%s, FUNC:%s, LINE:%d, to_flash:%d\n", __FILE__, __func__, __LINE__, to_flash);
	if (to_flash)
...
int sunxi_flash_stop(struct v4l2_subdev *sd)
{
	...
	if (!flash)
		return 0;
	printk("FILE:%s, FUNC:%s, LINE:%d, flash_mode:%d\n", __FILE__, __func__, __LINE__, flash->fl_info.flash_mode);
...
{
	printk("FILE:%s, FUNC:%s, LINE:%d, mode:%d\n", __FILE__, __func__, __LINE__, mode);
	if (fls_info == NULL) {
...
static ssize_t show_camera_flash_switch(struct device *dev,
                  struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "%d\n", flash_power_flag);
}

static ssize_t set_camera_flash_switch(struct device *dev,
                 struct device_attribute *attr,
                 const char *buf, size_t len)
{
	struct flash_dev *flash = dev_get_drvdata(dev);
	struct v4l2_subdev *sd = &flash->subdev;
	unsigned int val;


	val = simple_strtoul(buf, NULL, 16);
		printk("FILE:%s, FUNC:%s, LINE:%d, val:%d\n", __FILE__, __func__, __LINE__, val);
...
static DEVICE_ATTR(switch, 0660, show_camera_flash_switch, set_camera_flash_switch);
...
static int flash_probe(struct platform_device *pdev)
{
...
#if defined (CONFIG_YOUXUEPAI_BOARD) || defined (CONFIG_COWBOY_BOARD)
	flash->pwm_dev = pwm_request(PWM_INDEX, "sunxi-vin flash");
	if ((!flash->pwm_dev) || IS_ERR(flash->pwm_dev)) {
		vin_err("flash request pwm failed\n");
	} else {
		flash->period_ns = 1000 * 1000 * 1000 / 40; /* freq = 40 Hz */
		flash->duty_ns = flash->period_ns * 254 / 255;
		pwm_config(flash->pwm_dev, flash->duty_ns, flash->period_ns);
		pwm_set_polarity(flash->pwm_dev, PWM_POLARITY_INVERSED);
		flash->pmw_control_reg = ioremap(PWM_PULSE_CONTROL_REG, 4);
		if (!flash->pmw_control_reg)
			vin_err("flash ioremap PWM_PULSE_CONTROL_REG failed\n");
	}
	...
	ret = sysfs_create_group(&pdev->dev.kobj, &aw_attribute_group);
	if (ret)
		vin_err("%s sysfs_create_group fail!\n",__func__);
...
static int flash_remove(struct platform_device *pdev)
{
	...
	sysfs_remove_group(&pdev->dev.kobj, &aw_attribute_group);
...
#if defined (CONFIG_YOUXUEPAI_BOARD) || defined (CONFIG_COWBOY_BOARD)
	if (flash->pwm_dev) {
		pwm_free(flash->pwm_dev);
		if (flash->pmw_control_reg)
			iounmap(flash->pmw_control_reg);
	}
#endif
	kfree(flash);
	return 0;
