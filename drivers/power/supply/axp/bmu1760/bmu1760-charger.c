...
static int bmu1760_charger_init(struct axp_dev *axp_dev)
{
	...
	axp_regmap_read(axp_dev->regmap, BMU1760_CHIP_ID, &val);
	if (!(val & 0x20)) {
		if (bmu1760_config.pmu_batdeten)
			axp_regmap_set_bits(map, BMU1760_BATDET_EN, 0x08);
		else
			axp_regmap_clr_bits(map, BMU1760_BATDET_EN, 0x08);
	}
...
static int bmu1760_charger_probe(struct platform_device *pdev)
{
	...
	axp_regmap_read(axp_dev->regmap, BMU1760_CHIP_ID, &val);
	chg_dev->bmu_version_d = (val & 0x20);
