...
static int usb_in_yxp = 0;
int charge_type_yxp = 0;
static char *charge_type[] = {
  "NO_Charge","DC_Charge","USB_Charge"
};

//dancm@20210112 add charge led control function
struct regmap  *pmu_regmap_yxp;
static struct kobject *charge_yxp_kobj=NULL;

void charger_led_on_yxp(void)
{
    struct regmap *	regmap = pmu_regmap_yxp;
    regmap_update_bits(regmap, AXP803_OFF_CTL, 0x30, 0x30);   
}

void charger_led_off_yxp(void)
{
    struct regmap *	regmap = pmu_regmap_yxp;
    
    regmap_update_bits(regmap, AXP803_OFF_CTL, 0x30, 0x00);
}

void en_chgled_mode_yxp(int en)
{
   	struct regmap *	regmap = pmu_regmap_yxp;
   	if(en)
   	  regmap_update_bits(regmap, AXP803_OFF_CTL, 0x08, 0x08);
    else
      regmap_update_bits(regmap, AXP803_OFF_CTL, 0x08, 0x00);
}
...
//dancm@20210112 add the charge_led node of sysfs
static int charge_led_yxp = 0;
static ssize_t chgled_show(struct device *dev, struct device_attribute *attr, char *buf)
{
   return sprintf(buf, "%d\n",charge_led_yxp);
}
static ssize_t chgled_store(struct device *dev, struct device_attribute *attr,
...
static int chgled_en_yxp = 1;
static ssize_t chgled_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
   return sprintf(buf, "%d\n",chgled_en_yxp);
}
static ssize_t chgled_mode_store(struct device *dev, struct device_attribute *attr,
					   const char *buf, size_t size)
{
	int var;
...
{
	struct regmap *	regmap = pmu_regmap_yxp;
	int reg_value,vbus_valid;

    regmap_read(regmap, AXP803_STATUS, &reg_value);
	vbus_valid = !!(reg_value & AXP803_STATUS_VBUS_USED);
	if(!vbus_valid)
		 charge_type_yxp = 0;

  return sprintf(buf, "%s\n",charge_type[charge_type_yxp]);
}

static DEVICE_NOAH_ATTR(chgled, S_IRUGO | S_IWUGO | S_IWGRP,
		   chgled_show, chgled_store);
static DEVICE_NOAH_ATTR(chgled_mode, S_IRUGO | S_IWUGO | S_IWGRP,
		   chgled_mode_show, chgled_mode_store);
static DEVICE_ATTR(charge_type, 0444, charge_type_show, NULL);

static int charger_yxp_sysfs_init(struct platform_device *pdev)
{
    int BaRet ;
  
	charge_yxp_kobj = kobject_create_and_add("charge_yxp", &pdev->dev.kobj); 
	if (charge_yxp_kobj == NULL)
	{
		printk("\n charger_yxp_sysfs_init:subsystem_register failed\n");
		BaRet = -ENOMEM;
		goto kobject_create_failed;
	}

	BaRet = sysfs_create_file(charge_yxp_kobj, &dev_attr_chgled.attr); //charge_led
	if (BaRet) {
		printk("\n charger_yxp_sysfs_init: charge_led subsystem_register failed\n");
		goto sysfs_create_failed;
	}
	
	BaRet = sysfs_create_file(charge_yxp_kobj, &dev_attr_chgled_mode.attr); //chgled_mode
	if (BaRet) {
		printk("\n charger_yxp_sysfs_init:chgled_mode subsystem_register failed\n");
		goto sysfs_create_failed;
	}

	BaRet = sysfs_create_file(charge_yxp_kobj, &dev_attr_charge_type.attr); //charge_type
	if (BaRet) {
		printk("\n light_proximity_sensor_sysfs_init:charge_type subsystem_register failed\n");
		goto sysfs_create_failed;
	}
		return 0 ;
	
sysfs_create_failed:
	kobject_del(charge_yxp_kobj);
...
static int axp803_usb_power_probe(struct platform_device *pdev)
{
  ...
  pmu_regmap_yxp = usb_power->regmap;
