...
/******20210407@dancm battery_cap handler*******/
static struct kobject *battery_yxp_kobj=NULL;
extern int charge_type_yxp;
static int cap_factory = 0;
static int cap_factory_yxp = 0;
static int cap_temp = 100;
#define FULL_POWER_YXP 1
#if FULL_POWER_YXP
static int cap_start_yxp = 0;
static int count_temp;
static int cap_down_yxp = 0;
#endif

#define LOW_POWER_YXP 1
#if LOW_POWER_YXP
static int low_count_temp = 0;
static int lowcap_start_yxp = 0;
#endif
/************************************************/
...
static struct regmap *	pmu_regmap_yxp;
void power_off_yxp(void)
{
	  struct regmap *regmap = pmu_regmap_yxp;
	  printk("%s vbat < 3.3v\n",__func__);
  	regmap_update_bits(regmap, 0x32, 0x80, 0x80);  //power off when REG32H[7] set 1 
}
void recharging_start_yxp(void)
{
    struct regmap *regmap = pmu_regmap_yxp;	
    printk("@kevin %s\n",__func__);
    regmap_update_bits(regmap, 0x33, 0x80, 0x00);
    regmap_update_bits(regmap, 0x33, 0x80, 0x80);
}
...
static int axp803_get_rest_cap(struct axp803_bat_power *bat_power)
{
	...
	bat_power->charger_exist = ac_valid || vbus_valid;
	if((bat_power->charger_exist)&&(!rest_vol))
    	rest_vol = 1;
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
	  if(lowcap_start_yxp||cap_start_yxp)
		  val->intval = cap_temp;
		else
		  val->intval = axp803_get_rest_cap(bat_power);
		if(cap_factory_yxp)
		  val->intval = cap_factory;
		break;
...
static int axp803_bat_power_init(struct axp803_bat_power *bat_power)
{
    ...
    if(charge_type_yxp == 1)
  	    axp803_set_bat_chg_cur(bat_power, 1000);
	else
...
static int get_jeita_status(struct axp803_bat_power *bat_power)
{
	int debouce = 3;
	int cur_status = bat_power->jeita_status;
	int cur_temp;

	cur_temp = bat_power->bat_temp;

	if (cur_temp >= bat_power->jeita_t3) {
		cur_status = STATUS_HOT;
	} else if (cur_temp >= bat_power->jeita_t2) {
		if (cur_status != STATUS_HOT || cur_temp <= (bat_power->jeita_t3 - debouce)) {
			cur_status = STATUS_WARM;
		}
	} else if (cur_temp >= bat_power->jeita_t1) {
		if ((cur_status != STATUS_WARM || cur_temp <= (bat_power->jeita_t2 - debouce))
			&& (cur_status != STATUS_COOL || cur_temp >= bat_power->jeita_t1 + debouce)) {
			cur_status = STATUS_NORMAL;
		}
	} else if (cur_temp >= bat_power->jeita_t0) {
		if ((cur_status != STATUS_NORMAL || cur_temp <= (bat_power->jeita_t1 + debouce)) 
			&& (cur_status != STATUS_COLD || cur_temp >= (bat_power->jeita_t0 + debouce))) {
			cur_status = STATUS_COOL;
		}
	} else if (cur_temp < bat_power->jeita_t0) {
		cur_status = STATUS_COLD;
	} else {
		// no change.
	}
...
static int set_constant_voltage(struct axp803_bat_power *bat_power, int vol)
{
	int reg_val = 0;
	int ret = 0;
	struct regmap *map = bat_power->regmap;
	...
	reg_val <<= 5;
	regmap_update_bits(map, AXP803_CHARGE1, 0x60, reg_val);

	return ret;
}
static int adjust_charger(struct axp803_bat_power *bat_power, int cur_status)
{
	int ret = 0;
	int target_current = 0;
	int target_voltage = 4350;
	...
	switch (cur_status) {
		...
		case STATUS_NORMAL:
           if(charge_type_yxp == 1)
			  ...
			  target_current = bat_power->dts_info.pmu_runtime_chgcur;	
			break;
		case STATUS_WARM:
			target_current = 500;
			target_voltage = 4350;
	...
	if (cur_status != bat_power->jeita_status) {
		printk("%s:temp=%d, jeita_st=%d-%d,cur=%d,vol=%d\n", __func__,
			bat_power->bat_temp, cur_status, bat_power->jeita_status, target_current, target_voltage);
	}
#if 0
	// check cpu loading
	if (bat_power->cpu_use >= CPU_USING_THRESHOLD) {
		if (target_current > DCDC_ICL_LEVEL_1) {
			target_current = DCDC_ICL_LEVEL_1;
		}
		printk("%s: hevy load limit,cpu_use=%d cur=%d\n", __func__, bat_power->cpu_use, target_current);
	}
	// check if bat full
	if (bat_power->is_full == true) {
		target_current = 0;
	}

#endif
	// check over heat
	if (bat_power->thermal_status != 0) {
		bat_power->emergency_stop = true;
		target_current = 0;
		printk("%s: weijun bat temp overheat charging stop!!\n", __func__);
	} else {
		bat_power->emergency_stop = false;
	}

	axp803_set_bat_chg_cur(bat_power, target_current);
	//set_constant_voltage(bat_power, target_voltage);
	bat_power->jeita_status = cur_status;

	return ret;
}

static int jeita_init(struct axp803_bat_power *bat_power)
{
	int ret = 0;

	bat_power->jeita_t3 = 50;
	bat_power->jeita_t2 = 43;
	bat_power->jeita_t1 = 15;
	bat_power->jeita_t0 = 0;
	bat_power->jeita_status = STATUS_NORMAL;
	bat_power->emergency_stop = 0;
	bat_power->thermal_status = 0;

	return ret;
}
static int check_thermal_status(struct axp803_bat_power *bat_power)
{
	int ret = 0;
	int hot_limit, cold_limit;

	hot_limit = 50;
	cold_limit = 0;

	//bat_power->bat_temp = axp803_get_bat_temp(bat_power);

	if (bat_power->emergency_stop) {
		hot_limit = hot_limit - 2;
		cold_limit = cold_limit + 2;
	}

	if (bat_power->bat_temp >= hot_limit) {
		ret = BATT_OVERHEAT;
	} else if (bat_power->bat_temp <= cold_limit) {
		ret = BATT_COLD;
	} else {
		// do nothing
	}

	bat_power->thermal_status = ret;

	return ret;
}

static int get_bat_data(struct axp803_bat_power *bat_power)
{
	bool bat_det, bat_charging;
	bool ac_valid, vbus_valid;
	unsigned int reg_value;
	...
	//if soc invalid init again
//	if (bat_power->is_init == false) {
//		soc_init(bat_power);
//	}

	bat_power->bat_temp = axp803_get_bat_temp(bat_power);
//	bat_power->vbat = axp803_get_vbat(bat_power);
//	bat_power->ibat = axp803_get_ibat(bat_power) - axp803_get_disibat(bat_power);
//	bat_power->soc = axp803_get_rest_cap(bat_power);
	...
	/* ACIN or VBUS not present, bat must be present */
	if (!bat_det && ((reg_value & 0xF0) == 0x00))
		bat_det = true;

	//bat_power->bat_exist = bat_det;
	bat_power->charger_exist = ac_valid || vbus_valid;
	bat_power->is_charging = bat_charging;

	//printk("%s: %d,%d,%d\n", __func__,
	//	bat_power->charger_exist, bat_power->is_charging, bat_power->bat_temp);

	return 0;
}
#if FULL_POWER_YXP
void bat_cap_adjust_yxp(struct axp803_bat_power *bat_power)
{
	int rest_pct;
	int vbat;
	int ibat;
	...
	if(charge_type_yxp == 1)
		cutoff_current = 100;  //20210406@dancm  ac charger current = 1000mA,cutoff_current = 1000/10;
	else if(charge_type_yxp == 2)
		cutoff_current = bat_power->dts_info.pmu_runtime_chgcur/10;
	if( bat_power->charger_exist && ibat < -20 && !bat_power->is_charging&& rest_pct <= 99)
		recharging_start_yxp();
	if((rest_pct >90&&rest_pct <= 100 )&&(ibat >= 0 && ibat<=cutoff_current)&&(vbat > 4200)&&bat_power->charger_exist)
	{
	    if(!cap_start_yxp){
	      cap_temp  = rest_pct;
	      cap_down_yxp = 1;
  ...
  }else if(cap_start_yxp&& bat_power->charger_exist && ibat < -20 && !bat_power->is_charging &&(rest_pct < 100)){    	
  	  if(cap_temp <= 99)
  	  	   recharging_start_yxp(); 	
	 	  if(rest_pct < cap_temp){
	 	   	  count_temp++;
			    if(count_temp >= 6){
	            cap_temp--;
	            count_temp = 0;
	...
	}else if (cap_start_yxp&&(!bat_power->charger_exist)&&rest_pct < cap_temp){ 
            if(cap_down_yxp){
      	        count_temp = 0;
      	        cap_down_yxp = 0;
            }	   
...
#if LOW_POWER_YXP
void low_power_off_yxp(struct axp803_bat_power *bat_power)
{
    int vbat;
    int rest_pct;
    ...
    if((vbat < 3300)&&(rest_pct>=0)&&(!bat_power->charger_exist)){  	
     	 if(!lowcap_start_yxp){
     	 	  cap_temp= rest_pct;
     	 	  lowcap_start_yxp = 1;
     	 	  low_count_temp = 0;
...
static void axp803_bat_power_monitor(struct work_struct *work)
{
	...
	get_bat_data(bat_power);
	cur_jeita_status = get_jeita_status(bat_power);
	adjust_charger(bat_power, cur_jeita_status);
  
#if FULL_POWER_YXP
      bat_cap_adjust_yxp(bat_power); 
#endif
#if LOW_POWER_YXP	
      low_power_off_yxp(bat_power);
#endif
	power_supply_changed(bat_power->bat_supply);
	...
	if (unlikely(axp_debug_mask & AXP_CHG)) {
		...
		unsigned char temp_val[2];
		unsigned int reg_value;
		int vbat, ibat, rest_pct, ocv_pct, col_pct ,temp;
		...
		temp = axp803_get_bat_temp(bat_power);
		rest_pct = axp803_get_rest_cap(bat_power);
		...
		printk("vbat:%d, ibat:%d, rest_pct:%d, ocv_pct:%d, col_pct:%d,, temp:%d\n",
			vbat, ibat, rest_pct, ocv_pct, col_pct, temp);
...
static ssize_t battery_cap_show(struct device *dev, struct device_attribute *attr, char *buf)
{
   return sprintf(buf, "%d\n",cap_factory);
}
static ssize_t battery_cap_store(struct device *dev, struct device_attribute *attr,
					   const char *buf, size_t size)
{
	int battery_factory;
	int battery_cap;
	
	 if(2 != sscanf(buf, "%d %d", &battery_factory,&battery_cap)){
	 	printk("%s buf requre fail\n",__func__);
	 	return size;
	 	}
	 cap_factory_yxp = battery_factory;
	 cap_factory = battery_cap;
	return size;
}
static DEVICE_NOAH_ATTR(battery_cap, S_IRUGO | S_IWUGO | S_IWGRP,
		   battery_cap_show, battery_cap_store);
static int battery_yxp_sysfs_init(void)
{
    int BaRet ;
  
	battery_yxp_kobj = kobject_create_and_add("battery_yxp", NULL); 
	if (battery_yxp_kobj == NULL)
	{
		printk("\n battery_yxp_sysfs_init:subsystem_register failed\n");
		BaRet = -ENOMEM;
		goto kobject_create_failed;
	}

	BaRet = sysfs_create_file(battery_yxp_kobj, &dev_attr_battery_cap.attr); //battery_cap
	if (BaRet) {
		printk("\n battery_yxp_sysfs_init: charge_led subsystem_register failed\n");
