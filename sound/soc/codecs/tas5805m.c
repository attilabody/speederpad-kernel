...
int ali_get_odm_name(char odm_name[], int size)
{
	snprintf(odm_name, size, "S5A_5805_TL_TL");
	return 0;
}

int is_cust_board(void)
{
	return 0;
}
...
void sunxi_afe_disable_i2s_out_global(void)
{
    sunxi_daudio_i2s_out_control(0, 0);
}

void sunxi_afe_enable_i2s_out_global(void)
{
    sunxi_daudio_i2s_out_control(0, 1);
}
#else
extern void sunxi_afe_enable_i2s_out_global(void);
extern void sunxi_afe_disable_i2s_out_global(void);
#endif
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	if (tas5805m->control == OFF) {
		...
		sunxi_afe_enable_i2s_out_global();
		msleep(2);
		tas5805m_i2c_init(tas5805m,tas5805m->eqmode, 0);
		sunxi_afe_disable_i2s_out_global();
		enable_irq(tas5805m->irq);
	}

	if (tas5805m->eqmode == NORMAL) {
		if (!strcmp(odm_name, "S5A_5805_TL_TL")) {
			eq_start = tas5805m_get_eqindex(tas5805m_init_sequence_s5a_tl,
							ARRAY_SIZE(tas5805m_init_sequence_s5a_tl));
			eq_end = ARRAY_SIZE(tas5805m_init_sequence_s5a_tl) - 8;
			eq_ok = tas5805m_eqcheck(tas5805m, tas5805m_init_sequence_s5a_tl, eq_start, eq_end);
		} else if (!strcmp(odm_name, "S7A_5805_TL_TL")) {
                        eq_start = tas5805m_get_eqindex(tas5805m_init_sequence_s7a_tl,
                                                        ARRAY_SIZE(tas5805m_init_sequence_s7a_tl));
                        eq_end = ARRAY_SIZE(tas5805m_init_sequence_s7a_tl) - 8;
                        eq_ok = tas5805m_eqcheck(tas5805m, tas5805m_init_sequence_s7a_tl, eq_start, eq_end);
                } else {
...
static int get_speaker_param(char *odm_name, char *model, char *type)
{
        char str[21];
...
static int  tas5805m_i2c_init(struct tas5805m_priv *tas5805m,int eqmode,int prepare)
{
	...
	if(prepare){
        ...
        get_speaker_param(tas5805m->odmname, model, type);

        pr_info("%s : speaker type: %s, model:%s\n",__func__, type, model);

        if (!strcmp(model, "S5A")) {
	    if (!strcmp(type, "TL")) {
		pr_info("%s S5A write tongli eq!\n", __func__);
		ret = tas5805m_burst_write(tas5805m, tas5805m_init_regs_s5a_tl, ARRAY_SIZE(tas5805m_init_regs_s5a_tl));
            } else {
                pr_info("%s can not find odm name, write tongli eq default!\n",__func__);
		ret = tas5805m_burst_write(tas5805m, tas5805m_init_regs_s5a_tl, ARRAY_SIZE(tas5805m_init_regs_s5a_tl));
            }
        } else if (!strcmp(model, "S6")) {

        } else if (!strcmp(model, "S7A")) {
            if (!strcmp(type, "TL")) {
                pr_info("%s S7A write tongli eq!\n",__func__);
                ret = tas5805m_burst_write(tas5805m, tas5805m_init_regs_s7a_tl, ARRAY_SIZE(tas5805m_init_regs_s7a_tl));
            } else {
                pr_info("%s can not find odm name, write tongli eq default!\n",__func__);
                ret = tas5805m_burst_write(tas5805m, tas5805m_init_regs_s7a_tl, ARRAY_SIZE(tas5805m_init_regs_s7a_tl));
            }
            ...
            if (!strcmp(type, "TL")) {
                pr_info("%s default S5A write tongli eq!\n",__func__);
                ret = tas5805m_burst_write(tas5805m, tas5805m_init_regs_s5a_tl, ARRAY_SIZE(tas5805m_init_regs_s5a_tl));
            } else {
                pr_info("%s can not find odm name, write tongli eq default!\n",__func__);
                ret = tas5805m_burst_write(tas5805m, tas5805m_init_regs_s5a_tl, ARRAY_SIZE(tas5805m_init_regs_s5a_tl));
...
static int  tas5805m_i2c_init(struct tas5805m_priv *tas5805m,int eqmode,int prepare)
{
	...
	pr_info("%s(odm %s) eqmode:%d\n",__func__, odm_name, tas5805m->eqmode);
	if(tas5805m->eqmode == CHILD) {
		if (!strcmp(odm_name, "S1_5805_YD_GG")) {
			for(i=0;i< ARRAY_SIZE(tas5805m_child_sequence_gg);i++) {
				ret = snd_soc_write(tas5805m->codec, tas5805m_child_sequence_gg[i].reg,tas5805m_child_sequence_gg[i].def);
				if (ret != 0) {
					pr_err("Failed to initialize TAS5805M to child mode ret: %d\n",ret);
				}
			}
		} else {
			for(i=0;i< ARRAY_SIZE(tas5805m_child_sequence);i++) {
				ret = snd_soc_write(tas5805m->codec, tas5805m_child_sequence[i].reg,tas5805m_child_sequence[i].def);
				if (ret != 0) {
					pr_err("Failed to initialize TAS5805M to child mode ret: %d\n",ret);
				}
			}
	...
	if(tas5805m->eqmode == NORMAL) {
		if (!strcmp(odm_name, "S1_5805_YD_GG")) {
			for(i=0;i< ARRAY_SIZE(tas5805m_init_sequence_gg);i++){
				ret = snd_soc_write(tas5805m->codec, tas5805m_init_sequence_gg[i].reg,tas5805m_init_sequence_gg[i].def);
				if (ret != 0) {
...
static void tas5805m_power(struct tas5805m_priv *tas5805m,int value)
{
	if(is_cust_board()){
		...
		pr_info("%s pjcust 77 is NOT cust--return!\n",__func__);
		return;
	}
...
#ifdef CONFIG_SND_SOFT_AGC
void sunxi_adc_update_gain(bool in_playback);
#endif
...
#ifdef CONFIG_SND_SOFT_AGC
	sunxi_adc_update_gain(0);
#endif
	if (get_battery_status() && tas5805m->fb_off && (tas5805m->control == ON)) {
		disable_irq(tas5805m->irq);
		tas5805m_pdn_mute(tas5805m,ON);
		sunxi_afe_disable_i2s_out_global();
		tas5805m_power(tas5805m,OFF);
...
static int tas5805m_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	case FB_BLANK_POWERDOWN:
		mutex_lock(&tas5805m->lock);
                ...
                if (get_battery_status() && !tas5805m->in_playback && (tas5805m->control == ON)) {
                        disable_irq(tas5805m->irq);
                        tas5805m_pdn_mute(tas5805m,ON);
                        sunxi_afe_disable_i2s_out_global();
                        tas5805m_power(tas5805m,OFF);
...
static int parse_dt(struct device *dev,struct tas5805m_priv *tas5805m)
{
	...
	if(is_cust_board()){
		...
		pr_info("%s pjcust 88 is NOT cust!\n",__func__);
	}
...
#ifdef TAS5805_BOOT_MODE
static int tas5805m_power_supply(struct i2c_client *client,struct tas5805m_priv *tas5805m)
{
	int ret;
	unsigned  int volt;
	...
	tas5805m->dvdd = devm_regulator_get(&client->dev, "reg-vio18");
	if (IS_ERR(tas5805m->dvdd)) {
	    dev_err(&client->dev,"devm_regulator_get dvdd error\n");
	    return  -1;
	}

	/*get current voltage setting */
	ret = regulator_get_voltage(tas5805m->dvdd);
	pr_info("regulator_get_voltage ret:%d\n", ret);

	/* set voltage to 1.8V */
	ret = regulator_set_voltage(tas5805m->dvdd,1800000,1800000);
	if(ret != 0){
	    dev_err(&client->dev,"regulator_set_voltage failed %d\n",ret);
	    return ret;
	}

	/*get voltage setting  again */
	volt = regulator_get_voltage(tas5805m->dvdd);
	if(volt == 1800000) {
	    pr_info("regulator_get_voltage  test pass! volt:%d\n", volt);
	}

	ret = regulator_enable(tas5805m->dvdd);
	if(ret != 0){
...
#ifdef TAS5805_BOOT_MODE
	g_boot_mode = get_boot_mode();
	pr_info("%s g_boot_mode:%d\n",__func__,g_boot_mode);

	if ((g_boot_mode != KERNEL_POWER_OFF_CHARGING_BOOT) && (g_boot_mode != LOW_POWER_OFF_CHARGING_BOOT)){
		pr_info("%s this is NORMAL_BOOT!\n",__func__);
		tas5805m_power_supply(client,tas5805m);
		if(is_cust_board()){
			pr_info("%s pjcust 99 is cust!\n",__func__);
			if (get_battery_status()) {
				pr_info("%s get_battery_status WITH battery!\n",__func__);
				parse_dt(dev,tas5805m);
				tas5805m->eqmode = NORMAL;
			} else {
				pr_info("%s get_battery_status WITHOUT battery!\n",__func__);
			}
		} else {
			pr_info("%s pjcust 99 is NOT cust!\n",__func__);
			parse_dt(dev,tas5805m);
			tas5805m->eqmode = NORMAL;
		}

	}else{
		pr_info("%s this is CHARGING_BOOT!\n",__func__);
	}
#else
	/*strcpy(tas5805m->odmname, "S5A_5805_TL_TL");*/
	parse_dt(dev,tas5805m);
	tas5805m->eqmode = NORMAL;
#endif
