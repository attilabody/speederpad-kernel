...
void sunxi_daudio_i2s_out_control(int daudio_num, int onoff)
{
	struct sunxi_daudio_info *sunxi_daudio;
	sunxi_daudio = sunxi_daudio_global[daudio_num];
	if (!sunxi_daudio)
		return;
	...
	if (onoff) {
		...
		regmap_update_bits(sunxi_daudio->regmap, SUNXI_DAUDIO_CLKDIV,
			(SUNXI_DAUDIO_BCLK_DIV_MASK<<BCLK_DIV),
			(5<<BCLK_DIV));
		clk_set_rate(sunxi_daudio->pllclkx4, 24576000 * 4);
		clk_set_rate(sunxi_daudio->moduleclk, 24576000);
	}

	regmap_update_bits(sunxi_daudio->regmap,
		SUNXI_DAUDIO_CTL,
		(1<<CTL_TXEN), (onoff<<CTL_TXEN));
	/* global always on */
	/*sunxi_daudio_global_enable(sunxi_daudio, onoff);*/
}
EXPORT_SYMBOL(sunxi_daudio_i2s_out_control);
