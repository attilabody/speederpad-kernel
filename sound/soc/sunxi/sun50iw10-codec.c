...
extern int yxp_spk_type;
#define SPKGPIO_GRADE  2               //0 1.2W  1 1.1W  2 1.0W   3 0.9W   4 0.8W 
struct sunxi_codec_info *sunxi_codecbak = NULL;
static int yxp_first_flag = 0;
int yxp_spkpin_en(int en)
{
	struct codec_spk_config *spk_cfg = NULL;
	if(sunxi_codecbak == NULL)
		return 0;
	spk_cfg = &sunxi_codecbak->spk_config;
	//printk("===yxp_spkpin_en:%d\n",en);
	if(en)
	{
...
static int yxp_speak_onoff(struct codec_spk_config *spk_cfg,int en)
{
	printk("===yxp_speak_onoff:%d,%d\n",en,yxp_spk_type);
	if(en)
	{
		if(yxp_spk_type == 1)
		{	/*
			...
			{
				yxp_first_flag = 1;
				printk("===yxp_speak_onoff first:%d,%d,%d\n",en,yxp_spk_type,yxp_first_flag);
				return 0;	//防止开机后的“啪”音
			}
			*/
			//aw87519_audio_off();
				...
				struct snd_kcontrol *k, int event)
{
	...
	case	SND_SOC_DAPM_POST_PMU:
		...
		if (sunxi_codec->hp_plugin)
			break;
#endif
		if (spk_cfg->used) {
#ifdef CONFIG_SND_SOC_AW87XXX
			yxp_speak_onoff(spk_cfg,1);
#else
			gpio_set_value(spk_cfg->spk_gpio, spk_cfg->pa_level);
#endif		
		...
		struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	case 1:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			snd_soc_update_bits(codec, SUNXI_ADC_FIFOC,
				(0x3 << ADC_CHAN_EN), (0x2 << ADC_CHAN_EN));
		}
		break;
	case 2:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
	...
	if (ret >= 0) {
		...
		if (!gpio_is_valid(sunxi_codec->spk_config.spk_gpio)) {
			...
			if (ret) {
				...
				//yxp_speak_onoff(&sunxi_codec->spk_config,0);
#endif				
			}
		}
	} else {
