...
#if defined(CONFIG_EXTCON) && defined(CONFIG_EXTCON_SGM7220)
enum dectect_jack {
	PLUG_OUT = 0x0,
	PLUG_IN  = 0x1,
};

struct fsa8049_config {
	u32 en_gpio;
	u32 pa_msleep;
	bool used;
	bool pa_level;
...
struct sunxi_card_priv {
	...
	struct fsa8049_config fsa8049_config;
#endif
#ifdef CONFIG_SND_SUNXI_SOC_SUN50IW10_JACK
	struct snd_soc_codec *codec;
...
static void sunxi_check_hp_detect_status(struct work_struct *work)
{
	...
	pr_debug("[%s] detect_state=%d, mute_state=%d\n", __func__,
	...
	if (priv->detect_state == PLUG_OUT) {
		...
		snd_soc_update_bits(priv->codec, SUNXI_DAC_REG,
			...
			if(reg_val == 0) {
				pr_debug("headpphone plugin \n");
				snd_soc_update_bits(priv->codec, SUNXI_MICBIAS_REG,
					(0x1 << MICDETPL), (0x1 << MICDETPL));
			}
		}
...
static void sunxi_typec_hs_reg_init(struct sunxi_card_priv *priv)
{
	snd_soc_update_bits(priv->codec, SUNXI_HMIC_CTRL, (0xffff << 0),
			    (0x0 << 0));
	snd_soc_update_bits(priv->codec, SUNXI_HMIC_CTRL,
			(0x1f << MDATA_THRESHOLD),
			(0x0a << MDATA_THRESHOLD));
...
static irqreturn_t jack_typec_interrupt(int irq, void *dev_id)
{
	struct sunxi_card_priv *priv = dev_id;
	struct timeval tv;
	u32 tempdata = 0, regval = 0;
...
static int sunxi_card_dev_probe(struct platform_device *pdev)
{
	...
	ret = of_get_named_gpio(np, "FSA8049_EN", 0);
	if (ret >= 0) {
		priv->fsa8049_config.used = 1;
		priv->fsa8049_config.en_gpio = ret;
		if (!gpio_is_valid(priv->fsa8049_config.en_gpio)) {
			dev_err(&pdev->dev, "gpio-fsa8049-en is invalid\n");
			ret = -EINVAL;
		} else {
			pr_debug("gpio-fsa8049-en:%d\n",
					priv->fsa8049_config.en_gpio);
			ret = devm_gpio_request(&pdev->dev,
					priv->fsa8049_config.en_gpio, "FSA8049_EN");
			if (ret) {
				dev_err(&pdev->dev, "failed to request FSA8049_EN gpio\n");
				ret = -EBUSY;
			} else {
				gpio_direction_output(priv->fsa8049_config.en_gpio, 1);
				gpio_set_value(priv->fsa8049_config.en_gpio, 1);
			}
		}
	} else {
		priv->fsa8049_config.used = 0;
	}
	...
	ret = request_irq(priv->jackirq, jack_typec_interrupt,
		   0, "audio jack typec irq", priv);
	...
	if (of_property_read_bool(np, "extcon")) {
		...
		static const unsigned int sgm7220_cable[] = {
			EXTCON_USB,
			EXTCON_USB_HOST,
			EXTCON_JACK_HEADPHONE,  <--- index=2
			EXTCON_NONE,
		};
		*/
		if (priv->extdev->state & (1 << 2))
			priv->detect_state = PLUG_IN;
	}
